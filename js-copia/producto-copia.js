...
productos.forEach((producto) => {
  const productoElemento = document.createElement("div");    //crea un div donde incluye las listas
  ...
  productoElemento.innerHTML = `
    ...
    `;
  seccionProductos.appendChild(productoElemento);   //appenChild=mueve los elementos a la pag html
});
function cargarProductos() {
  ...
  productosOfertas.forEach((producto) => {
    ...
    productoElemento.innerHTML = `
            ...
            <button type="button">comprar</button>
        `;
    seccionOfertas.appendChild(productoElemento);  //appenChild=mueve los elementos a la pag html
  });
}
cargarProductos();
...
document.querySelectorAll(".almacen.productos .productos").forEach((productoElemento, index) => {
  ...
  button.addEventListener("click", () => {
    ...
    if (cantidad > 0 && productos[index].stock - cantidad >= 0) {
      ...
      productoElemento.querySelector(".stock").innerText=productos[index].stock;  //actualiza la visualizacion del stock
...
document.querySelectorAll(".almacen.ofertas .productos").forEach((productoElemento, index) => {
  ...
  button.addEventListener("click", () => {
    ...
    if (cantidad > 0 && productosOfertas[index].stock - cantidad >= 0) {
      ...
      productoElemento.querySelector(".stock").innerText=productosOfertas[index].stock;  //actualiza la visualizacion del stock
      alert("SU COMPRA FUE EXITOSA");
    } else {
      alert(
        "¡¡NO HAY SUFICIENTE STOCK!! Ingrese una cantidad menor o ingrese mayor a cero."
...
function comprarConDescuento(cantidad, index) {
  let totalDescuento =
    cantidad * productosOfertas[index].precio * (1 - productosOfertas[index].descuento);
...
function comprarCerveza(cantidad, index) {
  ...
  let totalAPagar =
    totalPromo + (cantidad % 2 === 1 ? productosOfertas[index].precio : 0);
